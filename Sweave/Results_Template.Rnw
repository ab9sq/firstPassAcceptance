...
%
\newcommand{\reportMonth}{Janurary }
...
The data is processed using R\footnote{RStudio is utilized as an IDE}, \Rvers. The only extension (package or library) utilized is
